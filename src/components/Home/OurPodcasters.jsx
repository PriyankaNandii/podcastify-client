...
const OurPodcasters = () => {
  ...
  const handleUnsubscribed = async (podcasterUid) => {
    try {
      const response = await axiosPublic.delete(
        `/unsubscribed?subscriberEmail=${user?.email}&podcasterUid=${podcasterUid}`
      );
            ...
            <div className="space-y-4 text-center divide-y dark:divide-gray-300">
              ...
              <div className="flex  justify-between items-center gap-1 p-2">
                {listOfMySubscriptionId.includes(podcaster._id) ? (
                  <button
                    onClick={() => handleUnsubscribed(podcaster.uid)}
                ...
                <div className="  text-white text-3xl">
                  {listOfMySubscriptionId.includes(podcaster._id) ? (
                    <LuBellRing title="You have subscribed" />
                  ) : (
                    <IoIosNotifications title="Please subscribe" />
                  )}
                </div>
              </div>
            </div>
