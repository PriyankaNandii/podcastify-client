...
const BASE_URL = "https://podcastify-server-delta.vercel.app";
  ...
  const startBroadcast = async () => {
    ...
    try {
      ...
      const response = await fetch(
        `${BASE_URL}/generate-token?channelName=${channelName}&uid=${uid}&role=publisher`
      );
      ...
      {!isBroadcasting ? (
        ...
        <div className="text-center">
          <p className="text-xl font-semibold mb-4">
            ...
            <span className="text-yellow-300">{channelName}</span>
          </p>
          <button
