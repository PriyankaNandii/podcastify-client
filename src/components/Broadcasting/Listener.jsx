...
const Listener = () => {
    ...
    useEffect(() => {
        ...
        websocket.onmessage = (event) => {
            ...
            if (message.type === "live-podcasters") {
                ...
                setBroadcasters((prev) => [...prev, { channelName: message.channelName, uid: message.uid }]);
            } else if (message.type === "remove-broadcaster") {
                setBroadcasters((prev) => prev.filter(broadcaster => broadcaster.uid !== message.uid));
    ...
    const startListening = async (broadcaster) => {
        try {
            ...
            const response = await fetch(`${BASE_URL}/generate-token?channelName=${broadcaster.channelName}&uid=${uid}&role=subscriber`);
            ...
            {isListening && (
                <div className="mt-6 p-4 bg-gray-100 rounded-lg shadow-lg">
                    <p className="text-lg font-semibold text-gray-800 mb-2">
                        ...
                        <span className="text-indigo-600">{selectedBroadcaster.channelName}</span>
                    </p>
                    <button
